...
- **벡터 인덱싱**: 수집된 텍스트는 의미 단위(Chunking)로 분할 후 Embedding 하여 Vector DB에 저장한다.
- **메타데이터 관리**: 원본 데이터의 메타데이터(작성자, 날짜, 채널, 티켓 상태, URL 등)를 함께 저장하여 필터링에 사용한다.

### 1.3. Retrieval & Generation (검색 및 답변)
- **유사도 검색**: 사용자 질문 벡터와 가장 유사한 문서를 검색한다.
- **Hybrid Search**: 정확도 향상을 위해 벡터 검색과 키워드 검색(BM25)을 결합하여 사용한다.
